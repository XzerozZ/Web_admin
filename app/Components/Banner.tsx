...
useEffect(() => {
  ...
  return (
    ...
    {lastData.error == 'something_went_wrong' && <div id="err_uni_news" className=' text-neutral flex px-10 bg-err py-5 rounded-xl text-lg'>Error : Failed to register. Something went wrong!</div>}
    ...
    {lastData.massage == 'Addarticla_success' && <div id="add_success" className=' text-neutral flex px-10 bg-primary py-5 rounded-xl text-lg'> Added article ID <div className=' underline font-bold mx-2'>{lastData.data.news_id}</div> successfully!</div>}
    {lastData.massage == 'EditArticle_success' && <div id="edit_success" className=' text-neutral flex px-10 bg-primary2 py-5 rounded-xl text-lg'> Edited article ID <div className=' underline font-bold mx-2'>{lastData.data}</div> successfully!</div>}
    {lastData.massage == 'Deletearticla_success' && <div id="delete_success" className=' text-neutral flex px-10 bg-accent py-5 rounded-xl text-lg'> Deleted article ID <div className=' underline font-bold mx-2'>{lastData.data}</div> successfully!</div>}
